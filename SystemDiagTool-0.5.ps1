...
function Invoke-Software {
    ...
    Write-LogSection "Third-Party Software" -NoScreen:$NoScreen
    try {
        $regPaths = @(
            "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*",
            "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*",
        ...
        $thirdPartySoftware = Get-ItemProperty -Path $regPaths -ErrorAction SilentlyContinue |
            Where-Object {
                $_.DisplayName -and
                ...
                $_.DisplayName -notmatch '\.NET' -and
                # Filter out noise — KB entries, drivers, empty publishers
                $_.DisplayName -notmatch '^KB\d+' -and
        ...
        if ($thirdPartySoftware.Count -eq 0) {
            ...
            Write-Log ("  {0,-45} {1,-20} {2,-25} {3,-12}" -f "Name", "Version", "Publisher", "Install Date") -NoScreen:$NoScreen
            Write-Log ("  {0,-45} {1,-20} {2,-25} {3,-12}" -f ("-"*45), ("-"*20), ("-"*25), ("-"*12))         -NoScreen:$NoScreen
            foreach ($sw in $thirdPartySoftware) {
                $name    = if ($sw.DisplayName.Length -gt 44)  { $sw.DisplayName.Substring(0,41)  + "..." } else { $sw.DisplayName  }
                $pub     = if ($sw.Publisher -and $sw.Publisher.Length -gt 24) { $sw.Publisher.Substring(0,21) + "..." } else { $sw.Publisher ?? "N/A" }
                $ver     = if ($sw.DisplayVersion) { $sw.DisplayVersion } else { "N/A" }
                $instDate = if ($sw.InstallDate -and $sw.InstallDate -match '^\d{8}$') {
                                "$($sw.InstallDate.Substring(0,4))-$($sw.InstallDate.Substring(4,2))-$($sw.InstallDate.Substring(6,2))"
                            } else { "N/A" }
                Write-Log ("  {0,-45} {1,-20} {2,-25} {3,-12}" -f $name, $ver, $pub, $instDate) -NoScreen:$NoScreen
            }
            Write-Log "" -NoScreen:$NoScreen
